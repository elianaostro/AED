...
typedef int (*hash) (const char* key, size_t size);
...
counting_filter_t* counting_filter_create(int size, int num_hashes, hash* hash_function){
    counting_filter_t* filter = malloc(sizeof(counting_filter_t));
    if (!filter) return NULL;

    filter -> size = size;
    filter -> num_hashes = num_hashes;
    filter -> hash_function = hash_function;
